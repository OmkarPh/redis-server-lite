...
func HandleConnection(conn net.Conn, redisConfig *config.RedisConfig, kvEngine *engine.KvEngine) {
	conn_id := uuid.New()
	...
	for {
		...
		requestType := core.ResolveRequestType(identifierBytes[0])
		slog.Debug(fmt.Sprint("Serve request", requestType))

		commands := core.ResolveCommands(*reader, requestType)
		...
		for _, command := range commands {
			...
			if action, ok := actions.RedisActions[command.Action]; ok {
				currCommandResponses, err := action.Execute(kvEngine, redisConfig, command.Args...)
				if err != nil {
...
func main() {
	...
	kvEngine := engine.NewKvEngine(redisConfig)
	...
	for {
		...
		go HandleConnection(conn, redisConfig, kvEngine)
	}
}

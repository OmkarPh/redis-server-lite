...
func (action *PingAction) Execute(kvEngine *engine.KvEngine, redisConfig *config.RedisConfig, args ...string) ([][]byte, error) {
	...
	if len(args) == 1 {
		return RedisActions[resp.ACTION_ECHO].Execute(kvEngine, redisConfig, args...)
	}
